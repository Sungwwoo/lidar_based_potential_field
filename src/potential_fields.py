...
class BasicAPF(Node):
    def __init__(self):
        ...
        try:
            ...
            scan = self.getLatestMsg("scan", LaserScan)
    ...
    def convert_frame(self, pose: list):
        ...
        try:
            ...
            self.get_logger().error("Total force transform failed")
            pass
        return
    ...
    def run(self):
        self.get_logger().info("Enabling APF")
        self.is_running = True

    def stop(self):
        self.get_logger().info("Disabling APF")
        self.is_running = False
    ...
    def CalcVelocity(self, u_total, u, goal_orientation, relative_goal, min_dist):
        ...
        angular_ratio = 1.1 * abs(target_ang) / self.vel_theta_max
        ...
        # TODO: Clip Velocity with max acceleration
...
class ClusteredAPF(BasicAPF):
    ...
    def CalcPotentialField(self, gx, gy, obstacles, dist_max):
        ...
        if self.num_obstacles != 0 and len(obstacles) != 0:
            uo_, rep_ = [], []
            # self.get_logger().info(str(self.num_obstacles))
            # self.get_logger().info(str(len(obstacles)))

            for i in range(0, self.num_obstacles):
                # self.get_logger().info("calculating %d of %d", i + 1, self.n_detected_obstacles)
            ...
            if self.num_obstacles == 1:
                ...
                for i in range(0, len(uo_)):
                    # self.get_logger().info("calculating %d of %d", i+1, len(uo_))
